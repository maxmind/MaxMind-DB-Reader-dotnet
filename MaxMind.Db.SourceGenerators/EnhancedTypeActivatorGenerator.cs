    ...
    public class EnhancedTypeActivatorGenerator : IIncrementalGenerator
    {
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            IncrementalValueProvider<KnownTypeSymbols> knownTypes = context.CompilationProvider
                ...
                .WithTrackingName("MMDB_KnownTypes");
            ...
            IncrementalValuesProvider<TypeActivatorSpec?> typeSpecs = context.SyntaxProvider
                ...
                .WithTrackingName("MMDB_TypeActivatorSpecs");

            // Add validation and diagnostics pipeline
            IncrementalValuesProvider<TypeActivatorSpec> validatedSpecs = typeSpecs
                .Where(static spec => spec != null)
                .Select(static (spec, _) => spec!)
                .WithTrackingName("MMDB_ValidatedSpecs");

            // Register source output with collected specs for batch processing
            context.RegisterSourceOutput(validatedSpecs.Collect(), GenerateActivators);
        }
            ...
            ImmutableArray<TypeActivatorSpec> typeSpecs)
        {
            try
            {
                var validSpecs = typeSpecs.ToList();
        ...
        /// <summary>
        /// Generates an error file and reports diagnostics for debugging generator issues.
        /// </summary>
        private static void GenerateErrorFile(SourceProductionContext context, Exception ex)
        {
            // Report diagnostic for the error (simplified for now)
            // TODO: Add comprehensive diagnostic descriptors later
